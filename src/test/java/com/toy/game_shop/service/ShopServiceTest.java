...
class ShopServiceTest {
    ...
    @Test
    @DisplayName("재고 포함 상세 조회 확인")
    void findByIdWithDetail(){
        Shop saved = shopService.addShop(newShop("shop1"));
        Item item = itemRepository.save(Item.builder().name("Sword").type(ItemType.WEAPON).build());
        shopStockRepository.save(ShopStock.builder().shop(saved).item(item).stock(5).price(100L).build());

        entityManager.flush();
        entityManager.clear();

        Shop found = shopService.findByIdWithDetail(saved.getId());

        Assertions.assertThat(found.getStocks().size()).isEqualTo(1);
    }

    @Test
    @DisplayName("미존재 id로 재고 포함 상세 조회 확인")
    void findByIdWithDetailNonId(){
        Assertions.assertThatThrownBy(()->shopService.findByIdWithDetail(-1L))
                .isInstanceOf(NoSuchElementException.class);
    }
